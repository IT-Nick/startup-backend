...
import com.droidablebee.springboot.rest.domain.Element;
...
public class PersonEndpoint extends BaseEndpoint {
	...
	public String handlePostRequest(@RequestBody Element requestDto) {
		// Обрабатываем запрос
		String result = processRequest(requestDto);
		return result;
	}

	private String processRequest(Element requestDto) {
		// Обрабатываем запрос и возвращаем ответ
		// Определяем к какому набору формул обращаться по полю element в JSON
		// Считаем формулы и возвращаем ответ в виде JSON результата
		return "Request processed successfully!";
	}

	@PreAuthorize("hasAuthority('SCOPE_" + PERSON_READ_PERMISSION + "')")
	@RequestMapping(path = "/v1/elements", method = RequestMethod.GET)
	@Operation(
			summary = "Получить рассчет формулы по элементу",
			description = "Определяем к какому набору формул обращаться по полю element в JSON. Считаем формулы и возвращаем ответ в виде JSON результата")
